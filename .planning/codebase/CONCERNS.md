...
> **Note on ordering:** This document is ordered by what matters most for the next phase of work (Phase II chatbot), not by category. Concerns that would immediately break or embarrass the chatbot are first. Concerns that put money or user trust at risk come next. Lower-stakes items are last. Fix in this order.

---

## 1. Phase II Blockers — Fix Before Enabling the Chatbot
...
- Issue: `db/migrations/002_seed_services.sql` seeds four services (AI Strategy Consultation $500, Custom AI Development from $5k, WhatsApp Automation $2k/mo, Discovery Call free). The live site offers "Okami Review" ($299) and "Discovery Call" (free). The seed data has no Okami Review entry.
...
- Fix approach: Replace seed rows to match current offerings (Okami Review $299, Discovery Call free).

**Chatbot is a stub — real AI never fires (by design, Phase II work):**
...
## 2. Money & Payments — Protect Revenue and User Trust
...
## 3. Visibility in Production — Know When Things Break
...
## 5. Security — Lower Risk, Still Worth Fixing
...
## 6. Code Hygiene — Low Stakes, Easy Wins
...
- Impact: Lead capture fails silently during staging/preview deployments where Beehiiv keys are not set.
